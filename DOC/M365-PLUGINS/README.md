...
`cms-m365tools` ist eine modulare Microsoft-365-Rechner- und Tool-Box für 365CMS. Produktive Module sind der **All-Module-Best-Practice-Kompass**, der **Power Platform Kosten-Kalkulator mit Well-Architected-Review**, der **Google Workspace ↔ Microsoft 365 TCO-Rechner**, der **M365 Storage-Bedarfs-Rechner**, der **M365 Backup-Kosten-Rechner**, die **Lizenz-Audit-Checkliste**, der **Microsoft-Preiserhöhung-Tracker**, der **Teams Phone-Lizenz-Berater**, der **On-Premise Exchange zu Exchange Online ROI-Rechner**, der **Frontline Worker Lizenz-Eignung-Check**, der **Copilot Pilot-Phase-Rechner**, der **AI Pack vs. Copilot Pro Vergleich**, der **Archive Mailbox Rechner**, die **M365 Lizenzmatrix**, die **M365 Add-on-Matrix**, der **Annual vs. Monthly Commitment Rechner**, der **M365 Add-On-Konfigurator**, der **M365-Lizenzvergleich**, der **M365-Lizenz-Berater**, der **Copilot ROI-Rechner**, der **Shared-Mailbox vs. Lizenz-Rechner** und der **Copilot Lizenz-Pflicht-Checker**. Ab `1.28.0` nutzt die Toolbox bevorzugt den Paketpreis-Seed aus `cms-m365lic` und macht Public-, Member- und Spezialpreise zentral pflegbar. Ab `1.29.1` verwaltet ein eigener `Landingpage Designer`-Unterpunkt Content-, Layout-, Farb- und Sichtbarkeitswerte zentral. Ab `1.29.3` verwaltet der Unterpunkt `Matrixen` die Read-only Lizenzmatrix und Add-on-Matrix gemeinsam mit Bereichs- und Design-Tabs. Ab `1.29.9` startet der öffentliche M365-Plugin-Content bündig ohne äußeren Theme-Header- oder Theme-Wrapper-Abstand; spätes Head-Critical-CSS und `m365tools-content-host` decken Theme-/Customizer-Overrides ab.
...
Ab `1.29.1` ist der `Landingpage Designer` ein eigener Admin-Unterpunkt. Er speichert Designerwerte in den Gruppen `landing-content`, `landing-layout`, `landing-colors` und `landing-visibility` und steuert Contentheader, Review-Texte, Seitenbreite, Header-Varianten, Kategorie-Navigation, Modulbox-Layouts, Box-Stil, Dichte, Rundungen, Abschnittsabstände, Farbpalette und sichtbare Card-Bestandteile.

Ab `1.29.2` ergänzt der Designer weitere Ausblendoptionen für Header-Texte, Header-Buttons, einzelne Kennzahlen, Kategorie-Köpfe, Review-Beschreibungen, Modultitel-Links, Tool-Buttons und inaktive Modulhinweise. Primär- und Sekundärbutton haben eigene Texte und Ziele; Modulbox-Buttons können wahlweise zur Toolseite, zum Primär-/Sekundärbutton-Ziel oder zu einer globalen Zieladresse führen. Contentheader-Stil, Ausrichtung, Button-Layout sowie separate Header- und Button-Farben werden ebenfalls zentral gespeichert.

Ab `1.29.3` ersetzt der Admin-Unterpunkt `Matrixen` die zwei separaten Modul-Unterpunkte für Lizenzmatrix und Add-on-Matrix. Die Tabs `Lizenzmatrix`, `Add-on-Matrix` und `Design` speichern Texte, Buttonziele, CTA, Bereichssichtbarkeit, Contentheader-Stil, Header-Ausrichtung, Button-Layout, Rundung und Farben. Globale Design-Schalter können Bereiche außerhalb der eigentlichen Tabellen wie Header, Introbereich, Druckaktion, CTA, Hinweise, Quellenstand, Add-on-Bereichsheader und Paketkarten abschalten.
