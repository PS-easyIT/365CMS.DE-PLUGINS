(function () {
    ...
    function initRotator(root) {
        var slides = Array.prototype.slice.call(root.querySelectorAll('[data-mas-sidebar-slide]'));
        if (slides.length <= 1) {
            return;
        }
        ...
        var prev = root.querySelector('[data-mas-sidebar-prev]');
        var next = root.querySelector('[data-mas-sidebar-next]');
        if (prev) {
    ...
    document.addEventListener('DOMContentLoaded', function () {
        document.querySelectorAll('[data-mas-sidebar-rotator]').forEach(initRotator);
    });
}());
