...
$hdr_from     = htmlspecialchars($settings['detail_header_bg_from']       ?? '#6d28d9');
$hdr_to       = htmlspecialchars($settings['detail_header_bg_to']         ?? '#a855f7');
$hdr_title    = htmlspecialchars($settings['detail_header_title_color']   ?? '#ffffff');
...
$badge_verified_bg    = htmlspecialchars($settings['design_badge_verified_bg']    ?? 'rgba(255,255,255,0.15)');
$badge_verified_color = htmlspecialchars($settings['design_badge_verified_color'] ?? '#ffffff');
?>
<style>
:root {
  --sp-primary:     <?= $primary ?>;
...
<div class="sp-single">
  ...
  <div class="sp-single-header">
    <div class="sp-sh-inner">
      <div class="sp-sh-avatar">
        <?php if ($photo): ?>
          <img src="<?= htmlspecialchars($photo) ?>" alt="<?= $full_name ?>">
        <?php else: ?>
          <div class="sp-sh-avatar-placeholder" style="background:<?= $av_grad ?>;"><?= htmlspecialchars($initials ?: '🎤') ?></div>
        <?php endif; ?>
      </div>
      <div class="sp-sh-text">

        <!-- Badges oben (wie expert-header) -->
        <div class="sp-sh-header-badges">
          <span class="sp-sh-avail-badge sp-avail-<?= htmlspecialchars($avail) ?>"><?= htmlspecialchars($avail_label) ?></span>
          <?php if ($is_verified): ?><span class="sp-sh-name-badge">✔ Verifiziert</span><?php endif; ?>
          <?php $show_mvp = !empty($settings['design_show_mvp_badge'] ?? '1'); ?>
          <?php if ($show_mvp && $is_featured): ?><span class="sp-sh-name-badge sp-badge-mvp">⭐ MVP</span><?php endif; ?>
        </div>

        <!-- Name & Position -->
        <h1><?= $full_name ?></h1>
        <?php if ($position): ?><p class="sp-sh-subject"><?= $position ?></p><?php endif; ?>

        <!-- Firma (mit Link-Logik) -->
        <?php if ($company): ?><p class="sp-sh-company">🏢
        ...
        <!-- Info-Chips (Standort, Reise, Audience, Sprachen, Formate) -->
        <div class="sp-sh-chips">
          <?php if ($city): ?>
            <span class="sp-sh-chip">📍 <?= $city ?><?= $country ? ', ' . $country : '' ?></span>
          <?php endif; ?>
          <?php if ($travel): ?>
            <span class="sp-sh-chip"><?= htmlspecialchars($travel_label) ?></span>
          <?php endif; ?>
          <?php if ($max_audience): ?>
            <span class="sp-sh-chip">👥 max. <?= (int)$max_audience ?> Pers.</span>
          <?php endif; ?>
          <?php if ($languages): ?>
            <span class="sp-sh-chip">🗣 <?= htmlspecialchars($languages) ?></span>
          <?php endif; ?>
          <?php foreach (array_slice($formats, 0, 3) as $f): ?>
            <span class="sp-sh-chip"><?= htmlspecialchars($fmt_labels[$f] ?? $f) ?></span>
          <?php endforeach; ?>
        </div>

        <!-- Social Icons im Header (wie expert-header) -->
        <?php $has_header_social = $linkedin || $xing || $twitter || $instagram || $youtube || $website || $github || $gitlab; ?>
        <?php if ($has_header_social): ?>
        <div class="sp-sh-social">
          <?php if ($linkedin): ?>
            <a href="<?= htmlspecialchars($linkedin) ?>" target="_blank" rel="noopener" class="sp-sh-social-icon" aria-label="LinkedIn">
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" width="15" height="15"><path d="M4.98 3.5C4.98 4.88 3.87 6 2.5 6S.02 4.88.02 3.5C.02 2.12 1.13 1 2.5 1s2.48 1.12 2.48 2.5zM.02 8.5H5V24H.02V8.5zm7.97 0h4.8v2.1h.07C13.7 9 15.44 8 17.6 8c5.2 0 6.16 3.43 6.16 7.88V24H19v-7.2c0-1.72-.03-3.93-2.4-3.93-2.4 0-2.78 1.87-2.78 3.81V24H8z"/></svg>
            </a>
          <?php endif; ?>
          <?php if ($xing): ?>
            <a href="<?= htmlspecialchars($xing) ?>" target="_blank" rel="noopener" class="sp-sh-social-icon" aria-label="XING">
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" width="15" height="15"><path d="M18.188 0c-.517 0-.741.325-.927.66 0 0-7.455 13.224-7.702 13.657.015.024 4.919 9.023 4.919 9.023.17.308.436.66.967.66h3.454c.211 0 .375-.078.463-.22.089-.151.089-.346-.009-.536l-4.879-8.916c-.004-.006-.004-.016 0-.022L22.139.756c.097-.191.097-.387.006-.535C22.056.078 21.894 0 21.686 0h-3.498zM3.648 4.74c-.211 0-.385.074-.473.216-.09.149-.078.339.02.531l2.34 4.05c.004.01.004.016 0 .021L1.86 16.051c-.099.188-.093.381 0 .529.085.142.247.22.455.22h3.514c.518 0 .731-.405.92-.73l3.671-6.471-2.342-4.052c-.17-.309-.436-.807-.978-.807H3.648z"/></svg>
            </a>
          <?php endif; ?>
          <?php if ($twitter): ?>
            <a href="https://twitter.com/<?= htmlspecialchars(ltrim($twitter,'@')) ?>" target="_blank" rel="noopener" class="sp-sh-social-icon" aria-label="X / Twitter">
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" width="15" height="15"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-4.714-6.231-5.401 6.231H2.744l7.73-8.835L1.254 2.25H8.08l4.253 5.622L18.244 2.25zm-1.161 17.52h1.833L7.084 4.126H5.117L17.083 19.77z"/></svg>
            </a>
          <?php endif; ?>
          <?php if ($instagram): ?>
            <a href="https://instagram.com/<?= htmlspecialchars(ltrim($instagram,'@')) ?>" target="_blank" rel="noopener" class="sp-sh-social-icon" aria-label="Instagram">
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" width="15" height="15"><rect x="2" y="2" width="20" height="20" rx="5" ry="5"/><path d="M16 11.37A4 4 0 1112.63 8 4 4 0 0116 11.37z"/><line x1="17.5" y1="6.5" x2="17.5" y2="6.5"/></svg>
            </a>
          <?php endif; ?>
          <?php if ($youtube): ?>
            <a href="<?= htmlspecialchars($youtube) ?>" target="_blank" rel="noopener" class="sp-sh-social-icon" aria-label="YouTube">
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" width="15" height="15"><path d="M23.495 6.205a3.007 3.007 0 00-2.088-2.088c-1.87-.501-9.396-.501-9.396-.501s-7.507-.01-9.396.501A3.007 3.007 0 00.527 6.205a31.247 31.247 0 00-.522 5.805 31.247 31.247 0 00.522 5.783 3.007 3.007 0 002.088 2.088c1.868.502 9.396.502 9.396.502s7.506 0 9.396-.502a3.007 3.007 0 002.088-2.088 31.247 31.247 0 00.5-5.783 31.247 31.247 0 00-.5-5.805zM9.609 15.601V8.408l6.264 3.602z"/></svg>
            </a>
          <?php endif; ?>
          <?php if ($website): ?>
            <a href="<?= htmlspecialchars($website) ?>" target="_blank" rel="noopener" class="sp-sh-social-icon" aria-label="Website">
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" width="15" height="15"><circle cx="12" cy="12" r="10"/><line x1="2" y1="12" x2="22" y2="12"/><path d="M12 2a15.3 15.3 0 014 10 15.3 15.3 0 01-4 10 15.3 15.3 0 01-4-10 15.3 15.3 0 014-10z"/></svg>
            </a>
          <?php endif; ?>
          <?php if ($github): ?>
            <a href="<?= htmlspecialchars($github) ?>" target="_blank" rel="noopener" class="sp-sh-social-icon" aria-label="GitHub">
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" width="15" height="15"><path d="M12 .297c-6.63 0-12 5.373-12 12 0 5.303 3.438 9.8 8.205 11.385.6.113.82-.258.82-.577 0-.285-.01-1.04-.015-2.04-3.338.724-4.042-1.61-4.042-1.61C4.422 18.07 3.633 17.7 3.633 17.7c-1.087-.744.084-.729.084-.729 1.205.084 1.838 1.236 1.838 1.236 1.07 1.835 2.809 1.305 3.495.998.108-.776.417-1.305.76-1.605-2.665-.3-5.466-1.332-5.466-5.93 0-1.31.465-2.38 1.235-3.22-.135-.303-.54-1.523.105-3.176 0 0 1.005-.322 3.3 1.23.96-.267 1.98-.399 3-.405 1.02.006 2.04.138 3 .405 2.28-1.552 3.285-1.23 3.285-1.23.645 1.653.24 2.873.12 3.176.765.84 1.23 1.91 1.23 3.22 0 4.61-2.805 5.625-5.475 5.92.42.36.81 1.096.81 2.22 0 1.606-.015 2.896-.015 3.286 0 .315.21.69.825.57C20.565 22.092 24 17.592 24 12.297c0-6.627-5.373-12-12-12"/></svg>
            </a>
          <?php endif; ?>
          <?php if ($gitlab): ?>
            <a href="<?= htmlspecialchars($gitlab) ?>" target="_blank" rel="noopener" class="sp-sh-social-icon" aria-label="GitLab">
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" width="15" height="15"><path d="M22.65 14.39L12 22.13 1.35 14.39a.84.84 0 01-.3-.94l1.22-3.78 2.44-7.51A.42.42 0 014.82 2a.43.43 0 01.58 0 .42.42 0 01.11.18l2.44 7.49h8.1l2.44-7.49a.42.42 0 01.11-.18.43.43 0 01.58 0 .42.42 0 01.11.18l2.44 7.51 1.22 3.78a.84.84 0 01-.3.94z"/></svg>
            </a>
          <?php endif; ?>
        </div>
        <?php endif; ?>
  ...
  </div><!-- /.sp-single-header -->

  <!-- ── Two-Column Body ────────────────────────────────────── -->
  <div class="sp-single-body">
    ...
    <div class="sp-main-col">
      ...
      <?php if ($bio): ?>
        <div class="sp-detail-section">
          <h2 class="sp-section-title">Über <?= htmlspecialchars($first ?: 'den Speaker') ?></h2>
          <div class="sp-bio-text sp-wysiwyg-content"><?= $bio ?></div>
        </div>
      <?php endif; ?>

      <?php if ($speaking_style): ?>
        <div class="sp-detail-section">
          <h2 class="sp-section-title">🎙️ Vortragsstil</h2>
          <p class="sp-meta-text"><?= nl2br(htmlspecialchars($speaking_style)) ?></p>
        </div>
      <?php endif; ?>

      <?php if (!empty($topics)): ?>
        <div class="sp-detail-section">
          <h2 class="sp-section-title">🏷️ Themen & Schwerpunkte</h2>
          <div class="sp-pills-row">
            <?php foreach ((array)$topics as $t):
              $tname = is_object($t) ? ($t->topic_name ?? '') : (string)$t;
              if (!$tname) continue;
            ?>
              <span class="sp-topic-pill"><?= htmlspecialchars($tname) ?></span>
            <?php endforeach; ?>
          </div>
        </div>
      <?php endif; ?>
      ...
      <?php if (!empty($_skills)): ?>
        <div class="sp-detail-section">
          <h2 class="sp-section-title">🛠️ Skills & Technologien</h2>
          <div class="sp-pills-row">
            <?php foreach ($_skills as $sk): ?>
              <span class="sp-skill-tag"><?= htmlspecialchars($_skill_labels[$sk] ?? $sk) ?></span>
            <?php endforeach; ?>
          </div>
        </div>
      <?php endif; ?>
      ...
      <?php if (!empty($formats)): ?>
        <div class="sp-detail-section">
          <h2 class="sp-section-title">🎤 Vortragsformate</h2>
          <div class="sp-pills-row">
            <?php foreach ((array)$formats as $f): ?>
      ...
      <?php if ($target_audience): ?>
        <div class="sp-detail-section">
          <h2 class="sp-section-title">🎯 Zielgruppe</h2>
          <p class="sp-meta-text"><?= nl2br(htmlspecialchars($target_audience)) ?></p>
        </div>
      <?php endif; ?>

      <?php if ($awards): ?>
        <div class="sp-detail-section">
          <h2 class="sp-section-title">🏆 Auszeichnungen & Referenzen</h2>
          <p class="sp-meta-text"><?= nl2br(htmlspecialchars($awards)) ?></p>
        </div>
      <?php endif; ?>

      <?php if (!empty($_recognitions)): ?>
        <div class="sp-detail-section">
          <h2 class="sp-section-title">🏅 Auszeichnungen & Rankings</h2>
          <?php foreach ($_recognitions as $grp => $items):
            if (empty($items) || !is_array($items)) continue;
          ?>
            <h4 class="sp-rec-group-title"><?= htmlspecialchars($_rec_group_labels[$grp] ?? $grp) ?></h4>
            <div class="sp-pills-row" style="margin-bottom:.75rem;">
              <?php foreach ($items as $item): ?>
                <span class="sp-format-pill"><?= htmlspecialchars($item) ?></span>
              <?php endforeach; ?>
            </div>
          <?php endforeach; ?>
        </div>
      ...
      <?php if (!empty($events)): ?>
        <div class="sp-detail-section">
          <h2 class="sp-section-title">📅 Events & Auftritte</h2>
          <div class="sp-ev-grid">
            ...
            ?>
              <div class="sp-ev-card">
                <div class="sp-ev-card-date"><?= $ev_date_fmt ?: '—' ?></div>
                <div class="sp-ev-card-title"><?= $ev_title ?: 'Event' ?></div>
                <div class="sp-ev-card-meta">
                  <?php if ($ev_location): ?><span>📍 <?= $ev_location ?></span><?php endif; ?>
                  <?php if ($ev_type):     ?><span>· <?= $ev_type ?></span><?php endif; ?>
                </div>
                <?php if ($ev_presence !== 'presence'): ?>
                  <span class="sp-ev-card-badge" style="background:#dbeafe;color:#1e40af;">
                    <?= htmlspecialchars($presence_labels[$ev_presence] ?? $ev_presence) ?>
                  </span>
                <?php endif; ?>
              </div>
            <?php endforeach; ?>
    ...
    <aside class="sp-sidebar-col">
      ...
      <div class="sp-sidebar-card">
        <h3 class="sp-sidebar-title">Kontakt & Buchung</h3>
        <?php if ($email): ?>
          <a href="mailto:<?= htmlspecialchars($email) ?>" class="sp-cta-btn">✉ Kontakt aufnehmen</a>
        <?php endif; ?>
        <?php if ($website): ?>
          <a href="<?= htmlspecialchars($website) ?>" target="_blank" rel="noopener"
             class="sp-cta-btn sp-cta-btn--secondary">🌐 Website besuchen</a>
        <?php endif; ?>
        <?php if ($phone): ?>
          <a href="tel:<?= htmlspecialchars($phone) ?>" class="sp-cta-btn sp-cta-btn--secondary">📞 <?= htmlspecialchars($phone) ?></a>
        <?php endif; ?>
      </div>

      <!-- Details -->
      <div class="sp-sidebar-card">
        <h3 class="sp-sidebar-title">Details</h3>
        <div class="sp-info-grid">
          <?php if ($company): ?>
            <div class="sp-info-item"><span class="sp-info-label">Unternehmen</span><span class="sp-info-value"><?= $company ?></span></div>
          <?php endif; ?>
          <?php if ($city || $country): ?>
            <div class="sp-info-item"><span class="sp-info-label">Standort</span><span class="sp-info-value">📍 <?= $city ?><?= ($city && $country) ? ', ' : '' ?><?= $country ?></span></div>
          <?php endif; ?>
          <?php if ($travel): ?>
            <div class="sp-info-item"><span class="sp-info-label">Reisebereitschaft</span><span class="sp-info-value"><?= htmlspecialchars($travel_label) ?></span></div>
          <?php endif; ?>
          <?php if ($fee_min || $fee_max): ?>
            <div class="sp-info-item">
              <span class="sp-info-label">Honorar</span>
              <span class="sp-info-value">
                💶 <?= $fee_min ? number_format((float)$fee_min,0,',','.') : '' ?><?= ($fee_min && $fee_max) ? '–' : '' ?><?= $fee_max ? number_format((float)$fee_max,0,',','.') . ' €' : '' ?>
              </span>
            </div>
          <?php endif; ?>
          <?php if ($languages): ?>
            <div class="sp-info-item"><span class="sp-info-label">Sprachen</span><span class="sp-info-value"><?= htmlspecialchars($languages) ?></span></div>
          <?php endif; ?>
          <?php if ($max_audience): ?>
            <div class="sp-info-item"><span class="sp-info-label">Max. Audience</span><span class="sp-info-value"><?= (int)$max_audience ?> Personen</span></div>
          <?php endif; ?>
          <?php if ($acad_title || $gender): ?>
            <div class="sp-info-item"><span class="sp-info-label">Ansprache</span><span class="sp-info-value"><?= htmlspecialchars(trim("$acad_title $gender")) ?></span></div>
          <?php endif; ?>
        </div>
      </div>

      <!-- Social Profile -->
      <?php if ($linkedin || $xing || $twitter || $instagram || $youtube): ?>
        <div class="sp-sidebar-card">
          <h3 class="sp-sidebar-title">Social & Profile</h3>
          <div class="sp-social-row">
            <?php if ($linkedin): ?>
              <a href="<?= htmlspecialchars($linkedin) ?>" target="_blank" rel="noopener" class="sp-social-icon sp-social-icon--lg" title="LinkedIn"><svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" fill="currentColor" viewBox="0 0 24 24"><path d="M4.98 3.5C4.98 4.88 3.87 6 2.5 6S.02 4.88.02 3.5C.02 2.12 1.13 1 2.5 1s2.48 1.12 2.48 2.5zM.02 8.5H5V24H.02V8.5zm7.97 0h4.8v2.1h.07C13.7 9 15.44 8 17.6 8c5.2 0 6.16 3.43 6.16 7.88V24H19v-7.2c0-1.72-.03-3.93-2.4-3.93-2.4 0-2.78 1.87-2.78 3.81V24H8z"/></svg></a>
            <?php endif; ?>
            <?php if ($xing): ?>
              <a href="<?= htmlspecialchars($xing) ?>" target="_blank" rel="noopener" class="sp-social-icon sp-social-icon--lg" title="XING">✖</a>
            <?php endif; ?>
            <?php if ($twitter): ?>
              <a href="https://twitter.com/<?= htmlspecialchars(ltrim($twitter,'@')) ?>" target="_blank" rel="noopener" class="sp-social-icon sp-social-icon--lg" title="X/Twitter"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" viewBox="0 0 24 24"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg></a>
            <?php endif; ?>
            <?php if ($instagram): ?>
              <a href="https://instagram.com/<?= htmlspecialchars(ltrim($instagram,'@')) ?>" target="_blank" rel="noopener" class="sp-social-icon sp-social-icon--lg" title="Instagram">📸</a>
            <?php endif; ?>
            <?php if ($youtube): ?>
              <a href="<?= htmlspecialchars($youtube) ?>" target="_blank" rel="noopener" class="sp-social-icon sp-social-icon--lg" title="YouTube">▶</a>
            <?php endif; ?>
          </div>
        </div>
      <?php endif; ?>

    </aside><!-- /.sp-sidebar-col -->
