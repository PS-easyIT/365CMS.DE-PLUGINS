...
<div class="pr-admin-shell">
    ...
    <div class="pr-panel-grid pr-panel-grid--wide">
        <div class="admin-card">
            ...
            <?php if (empty($promos)): ?>
                <div class="empty-state"><p class="pr-empty-icon">🎯</p><p><strong>Keine Promos vorhanden</strong></p><p class="pr-empty-text">Rechts kannst du direkt die erste Promo anlegen.</p></div>
            <?php else: ?>
                <div class="users-table-container">
                    <table class="users-table">
                        ...
                        <?php foreach ($promos as $item): ?>
                            <tr>
                                <td><a href="?edit=<?php echo (int) $item['id']; ?>" class="pr-admin-link"><?php echo htmlspecialchars((string) $item['title']); ?></a></td>
                                <td>
        ...
        <div class="admin-card">
            <div class="pr-panel-header"><div><h3><?php echo $promo ? '✏️ Promo bearbeiten' : '➕ Promo anlegen'; ?></h3><p>Lege Titel, CTA-Ziel und Platzierung in einem Schritt fest.</p></div></div>
            <form method="post" class="admin-form">
                ...
                <div class="pr-form-grid">
                    ...
                    <div class="form-group"><label class="form-label">Platzierung</label><select name="placement_id" class="form-control"><option value="0">Keine feste Platzierung</option><?php foreach ($placements as $placement): ?><option value="<?php echo (int) $placement['id']; ?>" <?php echo ((int) ($promo['placement_id'] ?? 0) === (int) $placement['id']) ? 'selected' : ''; ?>><?php echo htmlspecialchars((string) $placement['name']); ?> — <?php echo htmlspecialchars(CMS_Promos_Repository::instance()->get_theme_hook_label((string) ($placement['theme_hook'] ?? 'manual'))); ?></option><?php endforeach; ?></select></div>
                </div>
                <div class="pr-form-grid">
                    <div class="form-group"><label class="form-label">Status</label><select name="status" class="form-control"><?php foreach (['draft' => 'Entwurf', 'active' => 'Aktiv', 'paused' => 'Pausiert', 'archived' => 'Archiviert'] as $key => $label): ?><option value="<?php echo htmlspecialchars($key, ENT_QUOTES, 'UTF-8'); ?>" <?php echo (($promo['status'] ?? 'draft') === $key) ? 'selected' : ''; ?>><?php echo htmlspecialchars($label); ?></option><?php endforeach; ?></select></div>
