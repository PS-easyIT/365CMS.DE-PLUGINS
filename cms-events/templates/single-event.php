...
<div class="ev-single-v2">
  ...
  <header class="ev-hero-v2" style="background:linear-gradient(135deg,<?= $hdr_from ?> 0%,<?= $hdr_to ?> 100%);">
    <div class="ev-hero-v2__badges">
      <?php if ($is_featured): ?>
        <span class="ev-hero-v2__badge ev-hero-v2__badge--featured">⭐ Featured</span>
      <?php endif; ?>
      <?php if ($status === 'cancelled'): ?>
        <span class="ev-hero-v2__badge ev-hero-v2__badge--cancelled">🚫 Abgesagt</span>
      <?php endif; ?>
    </div>
    <div class="ev-hero-v2__inner">
      ...
      <div class="ev-hero-v2__meta">
        ...
        <div class="ev-hero-v2__pills">
          <?php if ($is_online): ?>
            <span class="ev-hero-v2__pill ev-hero-v2__pill--online">🌐 Online</span>
          <?php elseif ($city): ?>
            <span class="ev-hero-v2__pill">📍 <?= $city ?></span>
          <?php endif; ?>
          <?php if ($ev_time): ?>
            <span class="ev-hero-v2__pill">🕐 <?= htmlspecialchars($ev_time) ?> Uhr</span>
          <?php endif; ?>
          <?php if ($category): ?>
            <span class="ev-hero-v2__pill">📂 <?= $category ?></span>
          <?php endif; ?>
          <?php if ($show_price && $price_type === 'free'): ?>
            <span class="ev-hero-v2__pill ev-hero-v2__pill--free">✓ Kostenlos</span>
          <?php elseif ($show_price && $price > 0): ?>
            <span class="ev-hero-v2__pill"><?= number_format($price, 2, ',', '.') ?> <?= $price_cur ?></span>
          <?php endif; ?>
          <?php if (!empty($speakers)): ?>
            <span class="ev-hero-v2__pill ev-hero-v2__pill--speaker">🎤 <?= count($speakers) ?> Speaker</span>
          <?php endif; ?>
        </div>
      </div>
    </div>
  ...
  <div class="ev-bridge-v2">
    ...
    <div class="ev-bridge-v2__about">
      ...
      <?php elseif (!empty($image_url)): ?>
        <img src="<?= $image_url ?>" alt="<?= $title ?>" style="width:100%;border-radius:10px;object-fit:cover;max-height:260px;">
      <?php else: ?>
    ...
    <div class="ev-sec-v2">
      ...
      <div class="ev-speaker-grid-v2">
        <?php foreach ((array)$speakers as $sp):
          ...
          $spRole  = htmlspecialchars($sp->position ?? ($sp->role ?? ''));
          ...
          $avMap   = ['available'=>['✅','Verfügbar','#065f46','#d1fae5'],'limited'=>['⏳','Begrenzt','#92400e','#fef3c7'],'booked'=>['🚫','Ausgebucht','#991b1b','#fee2e2']];
          [$avIco,$avLbl,$avTxt,$avBg] = $avMap[$spAvail] ?? $avMap['available'];
        ?>
          <div class="ev-spk-row">
            <div class="ev-spk-row__top">
              ...
              <div class="ev-spk-row__info">
                ...
                <?php if ($spRole): ?><div class="ev-spk-row__sub">🎤 <?= $spRole ?></div><?php endif; ?>
                <?php if ($spCity):  ?><div class="ev-spk-row__sub">📍 <?= $spCity ?></div><?php endif; ?>
                <span class="ev-spk-row__avail" style="--avail-bg:<?= $avBg ?>;--avail-txt:<?= $avTxt ?>"><?= $avIco ?> <?= $avLbl ?></span>
              </div>
            </div>
            <?php if ($spId > 0): ?>
