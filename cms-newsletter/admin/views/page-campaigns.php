...
<div class="nl-admin-shell">
    ...
    <div class="nl-panel-grid nl-panel-grid--wide">
                ...
                <div class="users-table-container">
                    <table class="users-table">
                        ...
                        <?php foreach ($campaigns as $item): ?>
                            <tr>
                                <td><a href="?edit=<?php echo (int) $item['id']; ?>" class="nl-admin-link"><?php echo htmlspecialchars((string) $item['name']); ?></a></td>
                                <td><?php echo htmlspecialchars((string) ($item['template_name'] ?? '—')); ?></td>
                                <td><?php echo htmlspecialchars((string) (($item['segment_slug'] ?? '') !== '' ? $item['segment_slug'] : 'alle aktiven')); ?></td>
                                <td><span class="nl-soft-badge nl-soft-badge--<?php echo htmlspecialchars((string) ($item['status'] ?? 'draft')); ?>"><?php echo htmlspecialchars((string) ($item['status'] ?? 'draft')); ?></span></td>
        ...
        <div class="admin-card">
            <div class="nl-panel-header">
                <div>
                    <h3><?php echo $campaign ? '✏️ Kampagne bearbeiten' : '➕ Kampagne planen'; ?></h3>
                    ...
                    <div class="form-group">
                        ...
                        <select name="template_id" class="form-control">
                            ...
                            <?php foreach ($templates as $item): ?>
                                <option value="<?php echo (int) $item['id']; ?>" <?php echo ((int) ($campaign['template_id'] ?? 0) === (int) $item['id']) ? 'selected' : ''; ?>><?php echo htmlspecialchars((string) $item['name']); ?></option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                    <div class="form-group">
                    ...
                    <div class="form-group">
                        ...
                        <select name="status" class="form-control">
                            <?php foreach (['draft' => 'Entwurf', 'ready' => 'Versandbereit', 'scheduled' => 'Geplant', 'sent' => 'Versendet'] as $key => $label): ?>
                                <option value="<?php echo htmlspecialchars($key, ENT_QUOTES, 'UTF-8'); ?>" <?php echo (($campaign['status'] ?? 'draft') === $key) ? 'selected' : ''; ?>><?php echo htmlspecialchars($label); ?></option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                    <div class="form-group">
