...
<div class="nl-admin-shell">
    ...
    <div class="nl-panel-grid nl-panel-grid--wide">
                ...
                <div class="users-table-container">
                    <table class="users-table">
                        ...
                        <?php foreach ($subscribers as $item): ?>
                            <tr>
                                <td><a href="?edit=<?php echo (int) $item['id']; ?>" class="nl-admin-link"><?php echo htmlspecialchars((string) $item['email']); ?></a></td>
                                <td><?php echo htmlspecialchars(trim((string) (($item['first_name'] ?? '') . ' ' . ($item['last_name'] ?? ''))) ?: '—'); ?></td>
                                <td><?php echo htmlspecialchars((string) ($item['segment_slug'] ?: 'general')); ?></td>
                                <td><span class="nl-soft-badge nl-soft-badge--<?php echo htmlspecialchars((string) ($item['status'] ?? 'pending')); ?>"><?php echo htmlspecialchars((string) ($item['status'] ?? 'pending')); ?></span></td>
                                <td>
        ...
        <div class="admin-card">
            <div class="nl-panel-header">
                <div>
                    <h3><?php echo $subscriber ? '✏️ Abonnent bearbeiten' : '➕ Abonnent anlegen'; ?></h3>
                    ...
                    <div class="form-group">
                        ...
                        <select name="status" class="form-control">
                            <?php foreach (['pending' => 'Pending', 'active' => 'Aktiv', 'unsubscribed' => 'Abgemeldet', 'bounced' => 'Bounce'] as $key => $label): ?>
                                <option value="<?php echo htmlspecialchars($key, ENT_QUOTES, 'UTF-8'); ?>" <?php echo (($subscriber['status'] ?? 'pending') === $key) ? 'selected' : ''; ?>><?php echo htmlspecialchars($label); ?></option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                </div>
