...
final class CMS_M365CALCULATOR_Admin_Pages
{
    ...
    public function render_dashboard_page(): void
    {
        ...
        $csrfToken = class_exists('CMS\\Security')
            ? \CMS\Security::instance()->generateToken('m365calculator_admin_modules')
            : '';

        if (($_SERVER['REQUEST_METHOD'] ?? '') === 'POST' && ($_POST['action'] ?? '') === 'save_module_settings') {
            if (class_exists('CMS\\Security') && !\CMS\Security::instance()->verifyToken($_POST['csrf_token'] ?? '', 'm365calculator_admin_modules')) {
