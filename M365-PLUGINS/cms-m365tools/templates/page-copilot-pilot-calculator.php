...
<main class="phinit-plugin m365calc-page m365calc-copilot-pilot-page" id="copilot-pilot-rechner">
    ...
    <?php if (!empty($result['preflight_checklist'])): ?>
    <section class="phinit-card m365calc-result-card" aria-labelledby="m365pilot-preflight-title">
        <header class="m365calc-result-heading">
            <section>
                <p class="phinit-overline">Governance-Checkliste</p>
                <h2 id="m365pilot-preflight-title">Vor dem Pilotstart klären</h2>
                <p>Diese Punkte machen den Pilot belastbarer: nicht nur Lizenzen zuweisen, sondern Zielgruppe, Datenzugriffe, Enablement und Messung vorbereiten.</p>
            </section>
        </header>
        <section class="m365calc-tool-grid" aria-label="Preflight Punkte für den Copilot Pilot">
            <?php foreach ((array) ($result['preflight_checklist'] ?? []) as $item): ?>
            <?php if (!is_array($item)) { continue; } ?>
            <article class="phinit-note phinit-note--info">
                <h3><?php echo $esc($item['title'] ?? 'Prüfpunkt'); ?></h3>
                <p><?php echo $esc($item['text'] ?? ''); ?></p>
            </article>
            <?php endforeach; ?>
        </section>
    </section>
    ...
    <section class="m365calc-result-grid" aria-label="Messkriterien und Quellen">
        ...
        <article class="phinit-card">
            <h2>FAQ zur Copilot-Pilotphase</h2>
            <section class="m365calc-faq-list" aria-label="Häufige Fragen zur Copilot Pilotphase">
                <?php foreach ((array) ($result['faq'] ?? []) as $item): ?>
                <?php if (!is_array($item)) { continue; } ?>
                <details>
                    <summary><?php echo $esc($item['question'] ?? 'Frage'); ?></summary>
        ...
        <article class="phinit-note phinit-note--info m365calc-source-card">
            <h2>Quellenstand</h2>
            <p>Quellenprüfung: <?php echo $esc($result['meta']['source_checked'] ?? '2026-05-17'); ?>. <?php echo $esc($result['meta']['price_basis'] ?? 'Preisannahmen vor Beschaffung prüfen.'); ?></p>
            <details>
